...
| `postStartCommand` only | Simple | No 24h loop |
| Long-running `db-sync` container loop | Portable; same network; env-configurable | Must design halt/restart carefully |

**Decision**: Always-on **`db-sync`** service: check → merge → sleep `SYNC_INTERVAL_SECONDS` (default 86400). First merge attempt at startup after local DB is healthy.
...
**Decision**: Support both; **default halt = true** so a missing REST API stack does not look like a healthy infinite sync. Local `db` and app continue running when sync halts.

**Restart policy**: Prefer `restart: "no"` on `db-sync` so halt does not cause restart storms. Skip mode keeps the process alive inside the loop (no exit).
...
| Halt if primary missing? | Yes, configurable; default halt |
| Writable local? | Yes |
